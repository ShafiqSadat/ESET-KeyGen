...
class EsetRegister(object):
    ...
    def createAccount(self):
        ...
        if uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'id', 'cc-accept'))", max_iter=10,
               raise_exception_if_failed=False):
            console_log('Cookies successfully bypassed!', OK)
            time.sleep(
                1)  # Once pressed, you have to wait a little while. If code do not do this, the site does not count the acceptance of cookies
        else:
...
class EsetKeygen(object):
    ...
    def sendRequestForKey(self):
        ...
        if self.mode == 'ESET HOME':
            uCE(self.driver,
                f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'data-label', 'subscription-choose-trial-ehsp-card-button'))")
        elif self.mode == 'SMALL BUSINESS':
            uCE(self.driver,
                f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'data-label', 'subscription-choose-trial-esbs-card-button'))")
        try:
    ...
    def getLicenseData(self):
        ...
        uCE(self.driver, f"return {GET_EBAV}('div', 'data-r', 'license-detail-license-model-additional-info') != null",
            max_iter=10)
...
class EsetVPN(object):
    ...
    def sendRequestForVPNCodes(self):
        ...
        for profile in exec_js(
                f'return {GET_EBAV}("button", "data-label", "choose-profile-tile-button", -1)'):  # choose Me profile
            if profile.get_attribute("innerText").find(
                    self.email_obj.email) != -1:  # Me profile contains an email address
                profile.click()
        uCE(self.driver, f'return {CLICK_WITH_BOOL}({GET_EBAV}("button", "data-label", "choose-profile-continue-btn"))',
            max_iter=5)
        uCE(self.driver, f'return {GET_EBAV}("ion-button", "robot", "choose-device-counter-increment-button") != null',
            max_iter=10)
        ...
        uCE(self.driver, f'return {GET_EBAV}("button", "data-label", "pwm-instructions-sent-download-button") != null',
            max_iter=15)
    ...
    def getVPNCodes(self):
        if isinstance(self.email_obj, CustomEmailAPI):
            console_log('\nWait for a message to your e-mail about instructions on how to set up the VPN!!!', WARN,
                        True)
            return None
        else:
...
class EsetProtectHubRegister(object):
    ...
    def createAccount(self):
        ...
        console_log(f'\n{colorama.Fore.CYAN}Solve the captcha on the page manually!!!{colorama.Fore.RESET}', INFO,
                    False)
        while True:  # captcha
            try:
        ...
        try:
            ...
            raise RuntimeError(
                'ESET has blocked your IP or email, try again later!!! Try to use VPN or try to change Email API!!!')
        return True
...
class EsetProtectHubKeygen(object):
    ...
    def getLicenseData(self):
        ...
        try:
            ...
            license_key = exec_js(
                f'return {GET_EBAV}("div", "data-label", "license-overview-key-value").children[0].textContent').split(
                ' ')[0].strip()
            console_log('Information successfully received!', OK)
            return license_name, license_key, license_out_date
        except:
        ...
        if self.email_obj.class_name == 'custom':
            ...
            license_key, license_out_date, license_id = parseEPHKey(self.email_obj, self.driver, delay=5,
                                                                    max_iter=30)  # 2.5m
            console_log(f'License ID: {license_id}', OK)
            console_log('\nGetting information from the license...', INFO)
            console_log('Information successfully received!', OK)
